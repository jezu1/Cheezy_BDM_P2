...
def create_cuisines(spark_joined_rest):
    ...
    #give cuisines a cuisine_key using dense_rank
    windowSpec = Window.orderBy("cuisine")
    cuisine_tabel = cuisine_tabel.withColumn("cuisine_key", dense_rank().over(windowSpec) - 1)
...
def create_meals(spark_joined_rest):
    ...
    #give meals a meal_key using dense_rank
    windowSpec = Window.orderBy("meals")
    meals_tabel = meals_tabel.withColumn("meal_key", dense_rank().over(windowSpec) - 1)
